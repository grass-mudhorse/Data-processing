...
def exercise1(X_train, X_test, y_train, y_test, s):
    # create each model based on each Classification algorithm
    # mis_rate list is used to store every misclassification rate for each model
    ...
    for m in model:
        ...
        temp = 1-accuracy_score(y_test, y_pred)
        temp = round(temp, 3)
        mis_rate.append(temp)
    dic = {'knn': mis_rate[0], 'mlp': mis_rate[1], 'rf': mis_rate[2]}
    return dic
...
def exercise2(X_train, X_val, X_test, y_train, y_val, y_test, kvals):
    ...
    for k in kvals:
        ...
        mm = knn.fit(X_train, y_train)
        y_pred = mm.predict(X_val)
        temp = 1-accuracy_score(y_val, y_pred)
        temp = round(temp, 3)
        mis_rate.append(temp)
    mis_rate = pd.Series(mis_rate, index=kvals)
    # choose the best k value to test it on the test data set
    k_best = mis_rate.idxmin()
...
def exercise3(X_train, X_val, X_test, y_train, y_val, y_test, tree_vals, s):
    ...
    mis_rate = []
    for t in tree_vals:
        rf = RandomForestClassifier(random_state=s, n_estimators=t)
        mm = rf.fit(X_train, y_train)
        y_pred = mm.predict(X_val)
        temp = 1-accuracy_score(y_val, y_pred)
        temp = round(temp, 3)
        mis_rate.append(temp)
    mis_rate = pd.Series(mis_rate, index=tree_vals)
    # choose the best value of trees and test it on test data set
    t_best = mis_rate.idxmin()
    rf = RandomForestClassifier(n_estimators=t_best, random_state=s)
    mm = rf.fit(X_train, y_train)
    y_pred = mm.predict(X_test)
    temp = 1-accuracy_score(y_test, y_pred)
    temp = round(temp, 3)

    dic = {'Trees': t_best, 'MR': temp}
    return dic
...
def exercise4(X_train, X_val, X_test, y_train, y_val, y_test, layer_vals, s):
    ...
    mis_rate = []
    for l in layer_vals:
        mlp = MLPClassifier(random_state=s, hidden_layer_sizes=l)
        mm = mlp.fit(X_train, y_train)
        y_pred = mm.predict(X_val)
        temp = 1-accuracy_score(y_val, y_pred)
        temp = round(temp, 3)
        mis_rate.append(temp)
    mis_rate = pd.Series(mis_rate, index=layer_vals)
    # choose the best number of hidden layer sizes and test it on the test data set
    l_best = mis_rate.idxmin()
    mlp = MLPClassifier(random_state=s, hidden_layer_sizes=l_best)
    mm = mlp.fit(X_train, y_train)
    y_pred = mm.predict(X_test)
    temp = 1-accuracy_score(y_test, y_pred)
    temp = round(temp, 3)

    dic = {'Neurons': l_best, 'MR': temp}
    return dic
