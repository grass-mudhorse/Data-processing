...
def exercise1(df,rowinds,colinds):
    df_temp = df.iloc[rowinds,colinds]
    return df_temp
...
def exercise2(df,col,output_label,val1,val2):
    flag = (col > val1) & (col <val2)   #set a flag for boolean operators to figure out valid values
    df_temp = df.loc[flag,[output_label]]
    return df_temp
...
def exercise3(df):
    n = df.shape[0]     #get the number of rows in df
    m = np.eye(n)       #create a n*n matrix for initialization
    for i in range(n):
        for j in range(n):
            m[i][j] = math.pow(((df['carat'][i] - df['carat'][j])/0.8),2) + math.pow(((df['table'][i] - df['table'][j])/57),2)
    return m
# Suggested test
...
def exercise4(df):
    temp_list = list()
    n = df.shape[0]
    distance = exercise3(test_df_2)
    for i in range(n):
        temp_list.append(sum(distance[i]))   #use sum() function to calculate the sum of each row
    df['Dissimilarity'] = temp_list
    return df
