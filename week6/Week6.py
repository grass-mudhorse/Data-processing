...
def exercise2(df):
    # get 'mean', 'std', 'min', 'max','minloc', 'maxloc' from each column in df. Using vstack to 
    # combine them into a matrix 
    data = np.vstack((df.mean().T, df.std().T, df.min().T, df.max().T, df.idxmin().T, df.idxmax().T ))
    # create a dataframe with data, required indexes and column names.
    df1 = pd.DataFrame(data,index = ['mean','std','min','max','minloc','maxloc'],columns = df.columns)
    return df1
